...
public class DBUtil {
    ...
    public static List<Map<String, Object>> query(String tableName,
        ...
        return query(tableName, false, null, whereClause, whereArgs, null, null, null, null, null);
    }

    /**
     * 执行sql通过 Map<String, Object>限定查询条件查询
     * @param tableName 表名
     * @param equalWhereMap where等值查询
     * @param fuzzyWhereMap where查询模糊查询
     * @return List<Map < String, Object>>
     * @throws Exception
     */
    public static List<Map<String, Object>> query(String tableName,
                                                  Map<String, Object> equalWhereMap,
                                                  Map<String, Object> fuzzyWhereMap) throws SQLException {
        String whereClause = "";
        List<Object> whereArgs = new ArrayList<>();
        List<Object> fuzzyArgs = new ArrayList<>();

        if (equalWhereMap != null && equalWhereMap.size() > 0) {
            Iterator<String> iterator = equalWhereMap.keySet().iterator();
            while (iterator.hasNext()) {
                String key = iterator.next();
                whereClause += (whereArgs.isEmpty() ? "" : " AND ");
                whereClause += (key + " = ? ");
                whereArgs.add(equalWhereMap.get(key));
            }
        }

        if (fuzzyWhereMap != null && fuzzyWhereMap.size() > 0) {
            Iterator<String> iterator = fuzzyWhereMap.keySet().iterator();
            while (iterator.hasNext()) {
                String key = iterator.next();
                whereClause += (whereArgs.isEmpty() ? "" : " AND ");
                whereClause += (key + " like ? ");
                fuzzyArgs.add(fuzzyWhereMap.get(key));
            }
        }
        return query(tableName, false, null, whereClause, whereArgs.toArray(), fuzzyArgs.toArray(), null, null, null, null);
    }
    ...
    public static List<Map<String, Object>> query(String tableName,
        ...
        return executeQuery(sql, selectionArgs, fuzzyArgs);
    }
     ...
     * @throws SQLException SQL执行异常
     */
    public static List<Map<String, Object>> executeQuery(String sql, Object[] bindArgs, Object[] bindFuzzyArgs) throws SQLException {
        ...
        try {
            ...
            int fuzzyParameterIndex = bindArgs == null ? 0 : bindArgs.length;
            if (bindFuzzyArgs != null) {
                for (int i = 0; i < bindFuzzyArgs.length; i++) {
                    preparedStatement.setObject(fuzzyParameterIndex + 1, "%" + bindFuzzyArgs[i] + "%");
                }
            }
            System.out.println(getExecSQL(sql, bindArgs));
            /**执行sql语句，获取结果集**/
